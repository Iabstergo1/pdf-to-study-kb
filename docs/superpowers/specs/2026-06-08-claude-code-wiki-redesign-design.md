...
Claude Code（唯一 LLM；一次交互式会话，人手动触发 /ingest <source>，合规非自动化）
...
边界：Python CLI 做确定性预处理 + 后置门禁 + 索引 + 状态跟踪（不调 LLM、无状态图）；Claude `/ingest` 做唯一的 LLM 工作——读整源、织 wiki、跨页合并、概念归一。（状态机、两阶段发布、window 级续跑、并发锁见 §3.3。）
...
```text
profiled → converted → windowed → workorder_ready
  → ingest_waiting → ingesting → ingested
  → (lint) → published | lint_failed
lint_failed → ingest_waiting（修复后重跑）
任意阶段失败 → <stage>_failed（幂等重跑回到该阶段入口）
```

### 业务 SQLite 表（取代旧 unit/run 结构）

- `sources(source_id PK, domain, format, added_at, current_stage, current_status)`
- `source_stage_runs(id, source_id, stage, status, started_at, finished_at, input_hash, error)` — 阶段级幂等/恢复依据
...
- **新增**：`source-convert`（分层后端 + 难页渲染）、processing windows 生成器（确定性 TOC/页码/token 滑窗）、`/ingest` 命令 + source 级 work order 生成器、canonical registry、learning-quality lint、后置 adjacency/断链/重复检查；**确定性 Python CLI 编排（普通顺序脚本，扩展 `scripts/pipeline.py`）+ 业务 SQLite 状态跟踪 + `pipeline status`/`pipeline next` CLI**。
...
4. **P4 `/ingest` 命令 + source 级 work order 事务协议**（核心重构）。
