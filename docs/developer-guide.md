...
| `pipeline-workspace/reuses/<src>/` | external-vault-reuse 的不可变 manifest、mapping、origin state、origin/首次 target 页字节证据 | gitignore |
| `pipeline-workspace/reuse-reseals/<src>/<operation-id>/` | v1→v2 reseal 的 canonical transition、新旧 source 字节与完整旧 evidence 归档；operation id = source + old manifest SHA + new mapping SHA 的确定性摘要 | gitignore |
...
│   ├── legacy_revision.py       # adopted legacy 页 sidecar 修订 + 事件链 + 恢复
│   ├── source_reuse.py          # 跨 vault 来源复用 / reseal 证据
│   ├── locks.py                 # 单 vault 写锁
...
| query-session 契约 | `check-session --id <run_id> [--saved]` | `cmd_check_session` → 单层目录校验 + `query_session.check_session` | session 目录 | ok / 问题列表 | 路径型 run_id 拒绝；saved 模式缺 candidate/evidence/decision/授权、非 canonical/重复/结构或一一对应错误 → exit 非零 | `test_query_session*.py` | 准确 |
...
### 4.2.2 跨 Vault 来源复用（`reuse-source`）
...
失败恢复采用前滚，且 published 只存在于两端：

| 边界 | active evidence / source 页 | state | 重跑动作 |
|---|---|---|---|
| 新代暂存完成 | 旧 / 旧 | `reused/published` + 旧 SHA | 校验同一 operation，开始迁移 |
| `begin_reuse_reseal` 后 | 旧 / 旧 | `reused/running` + 旧 SHA | 继续归档旧 evidence |
| 旧 evidence 已归档 | 暂时缺失 / 旧 | `reused/running` + 旧 SHA | 把已验证的新 evidence 原子 rename 到 active |
| 新 evidence 激活 | 新 / 旧 | `reused/running` + 旧 SHA | 仅当 source 仍等于旧 canonical 字节时原子替换 |
| source/派生层完成 | 新 / 新 | `reused/running` + 旧 SHA | 重验 live PDF/origin/target 与归档，然后完成状态事务 |
| `complete_reuse_reseal` 后 | 新 / 新 | `reused/published` + 新 SHA | 只补幂等日志（若缺），随后 exact no-op |

因此没有“published 状态指向另一代 evidence/source”的窗口。`source_stage_runs.reused` 的 input/output hash 与 `artifacts.reuse_evidence.sha256` 只在最后一个 SQLite 事务一起换新；三类 ingest ledger 始终为 0。测试通过仅在 pytest 子进程可见的环境 failpoint 覆盖 prepare/begin/archive/activate/source/derived/finish 七个持久化边界，逐一证明重跑收敛。

进程内异常经 `finally` 释放 vault lock；OS 级硬退出可能保留锁行，这沿用全项目的锁契约：待其 stale 后先执行 `unlock`，再用同一 reseal 参数前滚，不允许为绕开锁而手改状态或证据。
...
`lint` 与 `vault-lint` 共同调用；它双向核对 operation 目录与 log 锚点、授权/pre/candidate/transition/
post/事件链及路径边界。证据损坏、孤儿锚点或未收尾 `committing` 都 hard fail；completed 后 live 的
合法演进只报 `post-legacy-revision-live-drift` warning。

恢复合同刻意不让有效期制造死锁：`valid_until` 只在 preparation 与首次进入 committing 之前校验；
committing 事件一旦存在，前滚与 rollback 均不再校验。正常崩溃用同一 request 幂等前滚；若 live 出现
既非 switch-pre 也非 post 的未知字节，必须用 `--expect-live-manifest` 精确声明当前 SHA，未知字节先归档
到 `conflicts/`，再显式 `--recover forward|rollback`。prepared 可 `--abort --apply`，所有终态重放为
byte/mtime no-op。

**已知边界：**

- `mode: revert` 是整文件恢复，只在目标 live 仍逐字节等于旧 operation post 时可用。页面后来被其他
  来源更新后必须拒绝 revert，改从当前 live 新建 `mode: edit` 前向修正；不能自动拼接“旧正文 + 当前
  frontmatter”并冒险抹掉后来贡献。
- committing operation 会阻断普通 `lint` / `vault-lint`，直到原 operation 前滚或回滚；这是避免其他
  发布越过半完成切换的安全门，不是可忽略 warning。
- 项目尚无 generation lineage。completed 后可以发现 live SHA 漂移，但不能只靠最终字节自动证明是哪次
  后续 ingest 造成；本合同只证明自己的 pre/post 与切换过程。
- 普通书源若已完成一轮并发布，而旧 workorder 快照早于 legacy 修订，后续 `check-write` 会以
  `disk hash changed since snapshot` 拒绝。合法出口是在该书源仍为 `lint/published` 时 `reopen`，按当前
  live 重建 workorder 后再 `ingest-start → check-write`；若当前已 `ingesting/running`，应 resume，不能
  为迁就测试放宽 `REOPENABLE`。
...
由 `state_store.SCHEMA` 创建，**8 张表**（`_ensure_round_columns` 对旧库幂等补 `round` 列）。
**谁创建**：`add-source` 起各 `cmd_*`；**稳定产物**（机器状态真值）。
**轮次 token 语义（2026-07-18 P1）**：`work_orders.round` 由 `record_work_order` 每次 +1（workorder/
reopen/重预处理换轮，lint 失败重试不换轮）；`window_reads.round` / `ingest_progress.round` 在落账时
盖当轮章，归属判断只做 round 相等——**旧行/升级前行 round=NULL 永不匹配任何轮**（旧账不代新轮，
同秒碰撞免疫）；lint 记账只认本轮 `status=finished` 的窗口行。
...
| `work_orders` | `source_id, path, registry_hash, write_scope_json, created_at, round`（轮次 token，record 时 +1） | ingest-start、check-write、`round_anchor`（window-done/lint 轮次判定） |
...
| `ingest_progress` | `id, source_id, window_id, input_hash, started_at, finished_at, status, write_set_json, proposal_set_json, error, round`（start_window 盖当轮章并清旧 write/proposal），`UNIQUE(source_id,window_id)` | 断点续跑、lint 归属（lifetime）与**本轮记账**（round 相等 + finished）、`ingest-stats` 聚合 |
| `window_reads` | `source_id, window_id, read_at, round`（show-window 落账盖当轮章），`UNIQUE(source_id,window_id)` | window-done fail-fast 本轮读窗校验、lint `window-unread-write`/`windows-unread`、`ingest-stats` 读窗审计 |
...
**来源复用旁路**同样不属于线性状态机：`reuse-source --apply` 在 evidence/source/派生全部成功后登记
`external-vault-reuse` 的 `reused/published`，只写一条 `reused/done` stage 与一条 `reuse_evidence`
artifact；三类 ingest 台账也必须全为 0。历史 origin/mapping/target snapshots 不可变，只有 target live
演进是 warning-only。

`reseal-source` 是该旁路的受控证据替换状态：开始事务只把 source 从 `reused/published` 暂降为
`reused/running`，仍保留旧 stage/artifact SHA；文件与派生层全部换代并复核后，完成事务才同时更新
stage/artifact SHA 并恢复 published。`next` 会把崩溃残留显示为 `resume/in-progress: reused`，正确恢复动作是
用同一 reseal 命令与 mapping/旧 manifest SHA 重跑，而不是 `fail`、`retract-source` 或普通 `reuse-source`。
...
| `sources/<src>.md` | ingest（LLM）/ adopt-vault / reuse-source（后两者确定性） | `source` | 普通来源摘要；legacy 接管为 `legacy-vault`，跨库复用为 `external-vault-reuse`，两者均直接 published。常规 ingest 中**必须存在**，缺失阻断发布（`source-page-missing`） |
...
| `test_legacy_revision_cli.py` | adopted revision 请求/候选/overlay lint、主状态零改、崩溃前滚/rollback/abort/revert、事件与日志防伪、live drift 分层、过期 committing 可恢复、published 书源旧快照经 reopen 刷新 |
...
| `test_arbitration.py` | 证据模型/候选/物化/闭环门（candidate·closure·windows-blockers 判定矩阵已 parametrize） |
...
| `test_ops_metrics_cli.py` | `ingest-stats` 代理指标聚合、`proposals-resolve` 精确/批量退场 + `--all-matching` 护栏 |
| `test_doctor_cli.py` | `window-done --writes-file` 参数拒绝矩阵、`reset-source` 状态推进（dry-run/apply/护栏） |
| `test_staging_clean_cli.py` | staging 三分类、`--apply` 双护栏、幂等（dry-run→apply→再 apply 顺序场景） |
| `test_page_rules.py` | 页正文规则原语，含 `extract_question_stems`/`extract_propositions` |
| `test_mineru_backend.py` | MinerU 归一（mock 注入，不依赖真实安装） |
