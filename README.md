...
```mermaid
flowchart TD
    U["你说：把这个 PDF 加进知识库，领域 X"] --> P["① 确定性预处理（零 LLM，幂等）<br/>add-source → profile → source-convert → windows → workorder"]
    P --> W["② LLM 写库（唯一 LLM，同一会话）<br/>按 window 读整源 / 难页图 → 写 status:proposed 页<br/>→ resolve-concept 归一 + 综合层"]
    W --> G{"③ lint 收尾门禁"}
...
│   └── resume_hint.py / resume-ingest.ps1       # 中断续跑（hook + OS 调度脚本）
...
4. **`pipeline.py` 用的 Python 解释器必须装齐依赖（`pyyaml` / `pymupdf`）。** 计划任务在独立进程中运行，不会继承你交互式 shell 里 `conda activate study-kb` 的环境——它默认调用 `PATH` 上的 `python`，而那很可能是缺依赖的 conda **base**，导致脚本第一步 `pipeline.py status` 就报 `ModuleNotFoundError` 而静默空转。请用以下任一方式指定正确解释器（三选一，按优先级）：
   - 注册任务时给脚本加 `-Python "<你的 study-kb 环境的 python 绝对路径>"` 参数（最干净，只影响该任务）；
   - 设环境变量 `STUDY_KB_PYTHON=<同上路径>`（脚本会优先读取）；
   - 直接改 `scripts/resume-ingest.ps1` 里 `$Python` 的本机 fallback 路径。
   > ⚠️ **改脚本路径属于本机配置，不要提交回仓库**：每台机器的 conda 路径不同，仓库内 `scripts/resume-ingest.ps1` 应保持通用（fallback 到 `PATH` 的 `python`）。若你在本地改了它，用 `git update-index --skip-worktree scripts/resume-ingest.ps1` 让改动不被提交（撤销：`--no-skip-worktree`），**切勿把你自己的 `D:\...\envs\study-kb\python.exe` 这类绝对路径推到远程**，否则会让其他使用者拿到一条对他们无效的硬编码路径。
...
3. 跑 python -m pytest tests -q（须全绿；含架构守卫 test_legacy_removed 与文档守卫 test_command_docs）
4. 若动了 skill：两套 skill 树须保持字节对等（skill-gate 会校验）
5. 提 PR，说明改了什么、为什么
```
