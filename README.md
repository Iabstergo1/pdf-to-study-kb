...
- **公式可能出错**：公式风险页由 LLM 读整页 PNG 转写 KaTeX，复杂上 / 下标、分数、矩阵存在转写错误风险；关键公式请对照 `pipeline-workspace/staging/<src>/assets/` 下的难页源图核实（发布正文不含源图，D-1）。
...
收尾 `lint` 是 **fail-closed** 的发布门，分两段**事务隔离**执行：先跑 **vault preflight**——复检全库已发布页的渲染安全旧伤（callout 类型/嵌套、数学分隔符、空题干），发现即阻断 promote、去重登记 `Review-Queue/vault-health-*.md`，但**不回滚当前批**（修旧页后直接重跑）；随后 **batch lint** 检查当前批，任一项不过才**回滚就地改动**、把违规项写进 `Review-Queue/`，绝不带病 promote。同一套渲染安全扫描可用 `vault-lint` 独立运行（可 CI）。kb-save 的保存走会话作用域门禁 `lint --source kb-save --session <run_id>`（页面带 `save_session` 身份标记，candidate 集缺失/身份不符即整体 fail-closed）。
...
| 孤儿页 | 页面既无 frontmatter 归属也不在任何 write_set（无主页） | 补 frontmatter 归属后重跑 |
| 未记账写入（`unaccounted-write`） | 本轮 proposed 的 topic/comparison/synthesis/overview 未入写作台账——ingest 的窗口 `--writes` 或 kb-save 的会话 `candidate_write_set.json`（`source_refs` 只定归属，不算记账） | 补台账后重跑 |
| 渲染安全（`callout-unknown` / `callout-nested-malformed` / `math-delimiter-nonobsidian` / `question-stem-empty`） | callout 类型白名单、嵌套须 `> > [!type]`（同级头渲染成字面量、折叠答案泄漏）、行内公式须 `$…$` 而非 `\(…\)`、自测题干非空——同一扫描也作为 vault preflight 复检已发布页 | 修格式后重跑（旧页违规不回滚当前批） |
| 旧模板复辟（`legacy-concept-scaffold`） | concept 页出现 ≥3 个已废除的旧骨架标题（成套复活；单个自然标题合法） | 改为散文组织后重跑 |
...
| 表格内裸竖线（`formula-table-pipe`） | 表格单元格里 `$...$` 含**裸 `|`**（会被当列分隔符撕碎表格、KaTeX 渲染失败；`\|` 是正确的转义写法） | 用 `\lvert\rvert`、转义成 `\|`、或把公式移出表格 |
| 表格内 wikilink 裸竖线（`table-wikilink-pipe`） | 表格单元格里 wikilink 的别名竖线未转义，会撕碎表格渲染 | 转义别名竖线（`\|`），或把链接移出表格放散文 |
...
| overview 仍是种子（`overview-seed`） | 产出 concept 却 `overview.md` 仍含 init-vault 种子占位（未真正重写过，常见于 lint 回滚后忘记重新应用） | 重写 overview 后重跑 |
| 来源台账缺失（`source-page-missing`） | 产出 concept 却没有 `sources/<src>.md` 台账页 | 补写来源摘要页后重跑 |
...
# 日常层：纯函数/直接模块测试的正向白名单（十几秒；精确计数以 pytest --collect-only 为准）
python -m pytest tests -q -m fast --basetemp=$bt
