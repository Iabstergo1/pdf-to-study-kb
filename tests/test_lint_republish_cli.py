...
def _account(tmp_path, sid, writes, window="w0000"):
    """合规记账三连（2026-07-17 规格）：本轮读窗 → window-start → window-done --writes。
    页面须已写盘（window-done 做台账↔磁盘对账 + 本轮读窗校验）。"""
    import json as _json
    assert _run(["show-window", "--source", sid, "--window", window], tmp_path).returncode == 0
    assert _run(["window-start", "--source", sid, "--window", window, "--hash", "h1"],
                tmp_path).returncode == 0
    r = _run(["window-done", "--source", sid, "--window", window,
              "--writes", _json.dumps(writes)], tmp_path)
    assert r.returncode == 0, r.stdout + r.stderr


def test_lint_pass_publishes_and_builds_all_derived_layers(tmp_path):
    # 一次成功发布生成全部派生产物（合并自 promote+index / graph HTML / graph-data+graph-lint /
    # quiz-index 四条，断言全保留）。页面用 GOOD_LESSON+自测题，让 quiz 断言同场景可验。
    import json
    ...
    idx = (vault / "index.generated.md").read_text(encoding="utf-8")
    assert "domains/misc/lessons/a.md" in idx
    log = (vault / "log.md").read_text(encoding="utf-8")
    assert "lint" in log and "note" in log

    # ② 知识图谱 v2.0（publish-isolated 钩子）：graph-data 契约 + 力导向 HTML + canvas 已移除，
    # 且 graph-lint 在真实 vault 上 returncode 0（A2 门禁 + 钩子集成不打断发布）。
    ...
    rg = _run(["graph-lint"], tmp_path)
    assert rg.returncode == 0, rg.stdout + rg.stderr

    # ③ quiz 派生阅读层：published 页题干 + 回链进索引，答案不泄露。
    qi = (vault / "quiz-index.generated.md").read_text(encoding="utf-8")
    assert "这一节的核心权衡是什么？" in qi
    assert "domains/misc/lessons/a.md" in qi
    assert "在直觉与依赖关系之间取舍" not in qi
...
def test_vault_preflight_blocks_without_rolling_back_current_batch(tmp_path):
    ...
    _account(tmp_path, "old", ["domains/misc/lessons/old.md"])
    ...
    _account(tmp_path, "new", ["domains/misc/lessons/new.md"])
...
def test_unaccounted_write_requires_ledger_not_just_source_refs(tmp_path):
    ...
    assert _run(["show-window", "--source", "s", "--window", "w0000"], tmp_path).returncode == 0
...
def test_foreign_source_refs_page_not_published_by_accounting_source(tmp_path):
    ...
    # A 把它错误地记进自己的 write_set（记账前须本轮读窗——归属判定不受读窗影响）
    assert _run(["show-window", "--source", "a", "--window", "w0000"], tmp_path).returncode == 0
...
def test_lint_scopes_to_own_source_pages(tmp_path):
    ...
    assert _run(["show-window", "--source", "b", "--window", "w0000"], tmp_path).returncode == 0
    ...
    assert _run(["window-done", "--source", "b", "--window", "w0000",
                 "--writes", '["domains/misc/lessons/b.md", "domains/misc/lessons/b2.md"]'],
                tmp_path).returncode == 0
    assert _run(["ingest-done", "--source", "b"], tmp_path).returncode == 0
...
def test_lint_blocks_on_unattributed_proposed(tmp_path):
    ...
    _account(tmp_path, "note", ["domains/misc/lessons/a.md"])   # 只记 a——orphan 保持双无
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    r = _run(["lint", "--source", "note"], tmp_path)
    ...
    # 修复归属还须补记账（本轮台账义务覆盖全部非 source 页）
    assert _run(["ingest-start", "--source", "note"], tmp_path).returncode == 0
    _account(tmp_path, "note",
             ["domains/misc/lessons/a.md", "domains/misc/lessons/orphan.md"])
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
...
def test_reopen_enables_incremental_publish(tmp_path):
    ...
    assert _run(["ingest-start", "--source", "note"], tmp_path).returncode == 0
    # 负向（2026-07-17 规格：旧轮 read 不得代新轮）：首轮读过 w0000，reopen 换锚后未重读
    # → window-done 拒收
    assert _run(["window-start", "--source", "note", "--window", "w0000", "--hash", "h"],
                tmp_path).returncode == 0
    r_stale = _run(["window-done", "--source", "note", "--window", "w0000"], tmp_path)
    assert r_stale.returncode != 0
    assert "show-window" in (r_stale.stdout + r_stale.stderr)
    # 本轮重读后继续增量（window id 取自 windows.jsonl——show-window 只认真实窗）
...
# reopen 的拒绝矩阵（未首轮入库/活动 ingest/未知源）在 test_state_store.py 的直接函数层；
# CLI wiring 的成功路径由 test_reopen_enables_incremental_publish 保留。
...
def test_lint_fail_blocks_rolls_back_and_queues(tmp_path):
    ...
    # 回滚可见性（并自 lists_rolled_back_files 条）：被还原的就地编辑在 stdout 与队列报告中列明
    # （重跑前需要重新应用的清单）。
    ...
    assert len(queue) == 1 and "L1" in qtext
...
def _setup_one_proposed(tmp_path, sid="note"):
    ...
    _account(tmp_path, sid, [f"domains/misc/lessons/{sid}.md"])
    assert _run(["ingest-done", "--source", sid], tmp_path).returncode == 0
    return tmp_path / "wiki"


# 成功路径的 graph/quiz 派生断言收拢在 test_lint_pass_publishes_and_builds_all_derived_layers；
# 这里保留的是 publish-isolated 的失败隔离契约（构建失败不得改变发布结果）。
...
def test_lint_blocks_concept_batch_without_source_page_and_seed_overview(tmp_path):
    ...
    assert _run(["window-start", "--source", "note", "--window", "w0000", "--hash", "h1"],
                tmp_path).returncode == 0
    assert _run(["window-done", "--source", "note", "--window", "w0000",
                 "--writes", '["topics/主题一.md", "domains/misc/concepts/概念甲.md"]'],
                tmp_path).returncode == 0
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    r2 = _run(["lint", "--source", "note"], tmp_path)
    assert r2.returncode == 0, r2.stdout + r2.stderr
...
def test_lint_blocks_write_in_unread_window(tmp_path):
    # lint 兜底（防"绕过 CLI 直写 DB 记账"）：本轮有写账却无本轮读窗 → window-unread-write。
    # 正常 CLI 路径已被 window-done fail-fast 拦住（见 orchestration 测试）——这里直接经
    # state_store 伪造账本行，模拟绕过 CLI 的记账（mimo 在 62/67 个窗上的做法的强化版）。
    db = _ingest_ready(tmp_path)
    vault = tmp_path / "wiki"
    mdpage.write_page(vault / "domains/misc/lessons/a.md",
                      {"type": "lesson", "status": "proposed", "managed_by": "pipeline",
                       "title": "A 课", "source": "note"}, GOOD_LESSON)
    state_store.start_window(db, "note", "w0000", input_hash="h1")
    state_store.finish_window(db, "note", "w0000",
                              write_set_json='["domains/misc/lessons/a.md"]')
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    r = _run(["lint", "--source", "note"], tmp_path)
    assert r.returncode != 0
    assert "window-unread-write" in (r.stdout + r.stderr)


def test_lint_accepts_write_in_window_that_was_read(tmp_path):
    _ingest_ready(tmp_path)
    vault = tmp_path / "wiki"
    mdpage.write_page(vault / "domains/misc/lessons/a.md",
                      {"type": "lesson", "status": "proposed", "managed_by": "pipeline",
...
def test_lint_blocks_unaccounted_concept_and_lesson(tmp_path):
    # 2026-07-17 规格 4：记账义务扩到全部非 source 页。concept/lesson 曾不受 unaccounted-write
    # 约束——不记账即可让读窗门禁完全看不见这些页（Codex 实跑复现的绕行路），从此关闭。
    _ingest_ready(tmp_path)
    vault = tmp_path / "wiki"
    concept_body = ("概念散文正文：先给直觉再给边界条件，说明它与相邻概念的关系与适用范围；"
                    "为稳定越过残次页兜底检查的字符下限再补一句，该概念的判定要件与反例各举其一，"
                    "以便读者能在不回读原文的情况下独立复核本页结论是否成立。\n")
    mdpage.write_page(vault / "domains/misc/concepts/未记账概念.md",
                      {"type": "concept", "status": "proposed", "managed_by": "pipeline",
                       "canonical_id": "concept.misc.wjz", "canonical_name": "未记账概念",
                       "domain": "misc", "source_refs": [{"source": "note"}]}, concept_body)
    mdpage.write_page(vault / "domains/misc/lessons/未记账课.md",
                      {"type": "lesson", "status": "proposed", "managed_by": "pipeline",
                       "title": "未记账课", "source": "note"}, GOOD_LESSON)
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    r = _run(["lint", "--source", "note"], tmp_path)
    assert r.returncode != 0
    out = r.stdout + r.stderr
    unaccounted = [ln for ln in out.splitlines() if "unaccounted-write" in ln]
    assert any("未记账概念" in ln for ln in unaccounted), out
    assert any("未记账课" in ln for ln in unaccounted), out


def test_old_round_ledger_does_not_account_new_round(tmp_path):
    # 2026-07-17 规格：旧轮账本不得代账。上一轮 write_set 里有同路径，reopen 换锚后该行是
    # 旧轮行——本轮 proposed 页必须有本轮记账，否则 unaccounted-write。
    db = _ingest_ready(tmp_path)
    vault = tmp_path / "wiki"
    # 旧轮：直写 DB 伪造一条含 comparisons/x.md 的账本行（文件此刻不存在，绕过 CLI 对账）
    state_store.start_window(db, "note", "w0000", input_hash="h0")
    state_store.finish_window(db, "note", "w0000", write_set_json='["comparisons/x.md"]')
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    # 收尾出 REOPENABLE 态（无 proposed 页；首轮读过 w0000 满足全窗必读 → lint 过）
    assert _run(["show-window", "--source", "note", "--window", "w0000"], tmp_path).returncode == 0
    assert _run(["lint", "--source", "note"], tmp_path).returncode == 0
    assert _run(["reopen", "--source", "note"], tmp_path).returncode == 0   # 换锚
    mdpage.write_page(vault / "comparisons/x.md",
                      {"type": "comparison", "status": "proposed", "managed_by": "pipeline",
                       "title": "X 对比", "source_refs": [{"source": "note", "sections": ["1"]}]},
                      COMPARISON_BODY)
    assert _run(["ingest-start", "--source", "note"], tmp_path).returncode == 0
    # 本轮只读窗、空写收窗（合法跳窗）——不给 x.md 记本轮账
    assert _run(["show-window", "--source", "note", "--window", "w0000"], tmp_path).returncode == 0
    assert _run(["window-start", "--source", "note", "--window", "w0000", "--hash", "h1"],
                tmp_path).returncode == 0
    ...
    assert r.returncode != 0
    out = r.stdout + r.stderr
    assert "unaccounted-write" in out and "comparisons/x.md" in out, out


def test_first_ingest_requires_reading_all_windows(tmp_path):
    # 2026-07-17 规格 2：首次完整入库 100% 全窗必读（空写跳窗也须读）——mimo 式
    # "读 5/67 窗发布全书"从此在 lint 处 fail-closed。
    assert _run(["init-vault"], tmp_path).returncode == 0
    note = tmp_path / "raw" / "two.md"
    note.parent.mkdir(parents=True, exist_ok=True)
    note.write_text("# A\n\naaa 内容\n\n# B\n\nbbb 内容\n", encoding="utf-8")
    for cmd in (["add-source", "--source", "two", "--domain", "misc", "--path", str(note),
                 "--fmt", "md"],
                ["profile", "--source", "two"], ["source-convert", "--source", "two"],
                ["windows", "--source", "two"], ["workorder", "--source", "two"],
                ["ingest-start", "--source", "two"]):
        r = _run(cmd, tmp_path)
        assert r.returncode == 0, f"{cmd}: {r.stderr}"
    mdpage.write_page(tmp_path / "wiki/domains/misc/lessons/a.md",
                      {"type": "lesson", "status": "proposed", "managed_by": "pipeline",
                       "title": "A 课", "source": "two"}, GOOD_LESSON)
    _account(tmp_path, "two", ["domains/misc/lessons/a.md"])   # 只读了 w0000
    assert _run(["ingest-done", "--source", "two"], tmp_path).returncode == 0
    r = _run(["lint", "--source", "two"], tmp_path)
    assert r.returncode != 0
    out = r.stdout + r.stderr
    assert "windows-unread" in out and "w0001" in out, out
    # 补读 w0001（空写跳窗，读了即可）→ 重跑通过
    assert _run(["show-window", "--source", "two", "--window", "w0001"], tmp_path).returncode == 0
    r2 = _run(["lint", "--source", "two"], tmp_path)
    assert r2.returncode == 0, r2.stdout + r2.stderr


def test_lint_blocks_out_of_scope_write_even_if_checkwrite_skipped(tmp_path):
    # 2026-07-17 规格 5：check-write 是写方自觉调用、可被跳过（Codex 实跑：DENY 后跳过直写
    # 照样发布）——lint 兜底复检 write_scope，越界 proposed 页阻断。
    _ingest_ready(tmp_path)
    vault = tmp_path / "wiki"
    out_page = vault / "domains/otherdomain/topics/越界主题.md"
    mdpage.write_page(out_page, {"type": "topic", "status": "proposed", "managed_by": "pipeline",
                                 "title": "越界主题",
                                 "source_refs": [{"source": "note"}]}, _TOPIC_BODY)
    _account(tmp_path, "note", ["domains/otherdomain/topics/越界主题.md"])
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    r = _run(["lint", "--source", "note"], tmp_path)
    assert r.returncode != 0
    out = r.stdout + r.stderr
    assert "write-scope-violation" in out and "越界主题" in out, out


def test_failed_window_does_not_resurrect_old_write_set(tmp_path):
    ...
    _account(tmp_path, "note", ["domains/misc/lessons/a.md"])
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    assert _run(["lint", "--source", "note"], tmp_path).returncode == 0      # 首轮发布
    assert _run(["reopen", "--source", "note"], tmp_path).returncode == 0    # 换轮
    assert _run(["ingest-start", "--source", "note"], tmp_path).returncode == 0
    assert _run(["show-window", "--source", "note", "--window", "w0000"], tmp_path).returncode == 0
    assert _run(["window-start", "--source", "note", "--window", "w0000", "--hash", "h2"],
                tmp_path).returncode == 0
    assert _run(["window-fail", "--source", "note", "--window", "w0000", "--error", "x"],
                tmp_path).returncode == 0
    meta, body = mdpage.read_page(vault / "domains/misc/lessons/a.md")
    meta["status"] = "proposed"                                              # 同路径重新入批
    mdpage.write_page(vault / "domains/misc/lessons/a.md", meta,
                      body + "\n新轮改写追加的一段，制造与首轮不同的 lint 输入。\n")
    assert _run(["ingest-done", "--source", "note"], tmp_path).returncode == 0
    r = _run(["lint", "--source", "note"], tmp_path)
    assert r.returncode != 0
    out = r.stdout + r.stderr
    assert "unaccounted-write" in out and "lessons/a.md" in out, out
