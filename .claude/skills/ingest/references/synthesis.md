...
- **overview.md 每源必更新**：把本源新概念挂进「核心概念地图」（并在其开头放**主题导航**块：用 wikilink 链到本源的 topic 页，使 overview 成为 topic→concept 的导航入口）、调整「推荐学习路线」、补「模型家族对比」。overview 是 living synthesis，**禁止退化成章节清单**（L5 会拦）。更新前 `check-write` + `snapshot-page`（它是已存在的 published 种子），改为 `status: proposed` 并进 `--writes`。
