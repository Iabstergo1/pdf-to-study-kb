...
- `pipeline-workspace/staging/<src>/{source.md, blocks.jsonl, chapters.json, parse_report.json, windows.jsonl, workorder.yaml, preflight_eval.json}` 与难页 PNG / 图表 asset。
...
- CLI：`init-vault`、`add-source`、`profile`、`source-convert`、`windows`、`workorder`、`preflight-eval`、`status`。
...
每步幂等；任一步报错就停止，不跳过。`preflight-eval` 只读既有 staging 产物、零 LLM；`--strict` 遇严重项非零退出，可挂 CI / 作为转 ingest 前的硬门。
