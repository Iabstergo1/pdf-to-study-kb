...
- **Lint hard rules (form is not policed; order/safety/provenance is):** wikilinks must be full
  ...
  pages as a transaction-isolated vault preflight); non-source pages (topic/comparison/synthesis/overview)
  must be accounted in the write ledger — a window's `--writes` (ingest) or the query-session
  `candidate_write_set.json` (kb-save); `source_refs` decides ownership, never accounting (`unaccounted-write`).
