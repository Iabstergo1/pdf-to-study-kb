...
def is_scanned_source(pages: list, *, zero_text_ratio: float = 0.8,
                      image_ratio: float = 0.8, text_floor: int = 50) -> bool:
    """整本扫描件判定（fail-closed gate）：≥80% 页几乎零文本层 且 ≥80% 页含图像。
    route B 适合 born-digital PDF 的少数难页，不适合让 LLM 临场 OCR 上千整页图——
    命中即应停在 profile、明确提示需 OCR route，不再生成可 ingest 的 workorder。
    少数扫描页混在普通 PDF（比值 < 0.8）不触发，仍按 route B 逐页处理，不误伤。"""
    n = len(pages)
    if not n:
        return False
...
def looks_like_numeric_grid(text: str) -> bool:
    ...
    for ln in text.splitlines():
        ...
        nums = _NUM.findall(s)
        if len(nums) >= 2 and (sum(c.isdigit() for c in s) / len(s)) >= 0.3:
            rows += 1                                  # 多数字 token + 数字密度高（CJK 散文密度低被排除）
    return rows >= 2                                   # 2×2 数值矩阵=2 数据行；漏页不可恢复故取低阈
...
def needs_vision_reasons(page: dict) -> list:
    ...
    elif (f >= 6 or page.get("eq_lines", 0) >= 2) and not is_code:
    ...
    # （n_draw≥6 或公式符号足够），后者补符号化矩阵（如用变量的支付矩阵），且避免"讨论矩阵思想"纯文本误报。
    matrix_struct = page.get("has_matrix_word", False) and (page.get("n_draw", 0) >= 6 or f >= 6)
    if page.get("n_tables", 0) >= 1 or page.get("numeric_grid", False) or matrix_struct:
        reasons.append("table")
    has_visual = (page.get("n_draw", 0) >= 8 or page.get("n_tables", 0) >= 1
                  or page.get("image_count", 0) >= 1)
...
def vision_tier(page: dict, reasons: list | None = None) -> str:
    ...
    if ("formula" in reasons or "scanned-or-image" in reasons
            or page.get("n_tables", 0) >= 1 or page.get("n_draw", 0) >= 20):
        return "must"
    return "nice"
