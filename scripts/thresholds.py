...
TOPIC_MONOPOLY_RATIO = _float("STUDY_KB_TOPIC_MONOPOLY_RATIO", 0.6)  # 单 topic 收编域内概念占比≥ → 软警告（倾倒征兆）
...
# ── knowledge graph v2（派生阅读层；验收期，不折进缓存键）──
GRAPH_DENSE_DEGREE = _int("STUDY_KB_GRAPH_DENSE_DEGREE", 25)  # graph-lint：单节点 degree> → 过密 warn
