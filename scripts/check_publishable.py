...
豁免：确实需要在文档里写示例路径时，在该行加行内标记 ``publishable-allow``。
豁免必须逐行显式声明，不提供目录级或模式级的整体豁免——那会让门禁悄悄失效。
"""
...
def main(argv=None) -> int:
    ...
    failed = errors or (warnings and args.strict)
    if not failed:
        print(f"[OK] check-publishable: 0 errors, {len(warnings)} warnings")
        return 0

    print(
        f"\n[FAIL] check-publishable: {len(errors)} errors, {len(warnings)} warnings"
        f"\n确需保留示例路径时，在该行加行内标记 {ALLOW_MARKER!r}；"
